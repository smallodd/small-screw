 ...
 */
package cn.smallodd.screw.core.query.highgo;

import cn.smallodd.screw.core.exception.QueryException;
import cn.smallodd.screw.core.mapping.Mapping;
import cn.smallodd.screw.core.metadata.Column;
import cn.smallodd.screw.core.metadata.Database;
import cn.smallodd.screw.core.metadata.PrimaryKey;
import cn.smallodd.screw.core.query.AbstractDatabaseQuery;
import cn.smallodd.screw.core.query.highgo.model.HigHgoColumnModel;
import cn.smallodd.screw.core.query.highgo.model.HigHgoDatabaseModel;
import cn.smallodd.screw.core.query.highgo.model.HigHgoPrimaryKeyModel;
import cn.smallodd.screw.core.query.highgo.model.HigHgoTableModel;
import cn.smallodd.screw.core.util.Assert;
import cn.smallodd.screw.core.util.CollectionUtils;
import cn.smallodd.screw.core.util.ExceptionUtils;
import cn.smallodd.screw.core.util.JdbcUtils;
...
import static cn.smallodd.screw.core.constant.DefaultConstants.PERCENT_SIGN;
...
public class HigHgoDataBaseQuery extends AbstractDatabaseQuery {
     ...
     */
    @Override
    public List<HigHgoTableModel> getTables() {
    ...
    public List<HigHgoColumnModel> getTableColumns(String table) throws QueryException {
        ...
        try {
            ...
            List<String> tableNames = list.stream().map(HigHgoColumnModel::getTableName)
    ...
    public List<? extends PrimaryKey> getPrimaryKeys() throws QueryException {
        ...
        try {
            ...
            String sql = "SELECT result.TABLE_CAT, result.TABLE_SCHEM, result.TABLE_NAME, result.COLUMN_NAME, result.KEY_SEQ, result.PK_NAME FROM(SELECT NULL AS TABLE_CAT, n.nspname AS TABLE_SCHEM, ct.relname AS TABLE_NAME, a.attname AS COLUMN_NAME, (information_schema._pg_expandarray(i.indkey)).n AS KEY_SEQ, ci.relname AS PK_NAME, information_schema._pg_expandarray(i.indkey) AS KEYS, a.attnum AS A_ATTNUM FROM pg_catalog.pg_class ct JOIN pg_catalog.pg_attribute a ON (ct.oid = a.attrelid) JOIN pg_catalog.pg_namespace n ON (ct.relnamespace = n.oid) JOIN pg_catalog.pg_index i ON (a.attrelid = i.indrelid) JOIN pg_catalog.pg_class ci ON (ci.oid = i.indexrelid) WHERE true AND n.nspname = '"+schema+"' AND i.indisprimary) result where result.A_ATTNUM = (result.KEYS).x ORDER BY result.table_name, result.pk_name, result.key_seq";
            // 拼接参数
